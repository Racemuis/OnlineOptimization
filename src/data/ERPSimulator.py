...
class DataSimulator(Source):
    ...
    def __init__(
        ...
        data_config: dict,
        bo_config: dict,
        noise_function: Optional[bool] = False,
        ...
        ), "Only the dimensionalities of 1, 2, 3 and 7 are supported."
        super().__init__()
        ...
        print(f"Reading data from subject {self.participant}...", end="", flush=True)
    ...
    def single_sample(self, x: np.ndarray, noise: bool, cv: bool) -> float:
        ...
        if noise and self.noise_function:
            ...
            noise_scale = 0.5 * np.abs(np.sin(x_scaled))
