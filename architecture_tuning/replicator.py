...
def main():
    """
     Find the optimal replicator by comparing none, variance and sequential.
    ...
    for p in participants:
        ...
        for r in tqdm(replicators, desc="Optimizing replicator"):
            replicates = np.zeros(conf["n_runs"])
            # Reset seeds
            np.random.seed(44)
            random.seed(44)
            ...
            for run in range(conf["n_runs"]):
                ...
                for i in range(conf["random_sample_size"] + conf["informed_sample_size"]):
                    ...
                    append_locations(
                        ...
                        location=np.array(sample).squeeze(),
                    )
                ...
                if opt.replicator is not None:
                    replicates[run] = opt.replicator.replicates

            print(f"Replicator {r} - average number of replications: {np.mean(replicates)}")
